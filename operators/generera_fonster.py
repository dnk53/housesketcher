...
# OPERATOR - GENERERA FÖNSTER (som komponent + placera i vägg)
# ________________________________________________________________________________________________
...
class MESH_OT_bt_skapa_fonster(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        comp_collection = komponent_fonster.create_window_component(
            context,
            name=name,
            W=p.bredd,
            H=p.hojd,
        ...
        if placed_count > 0 and last_placed:
            ...
            self.report({'INFO'}, f"Skapade och placerade {placed_count} fönsterkomponent(er)")
        else:
            self.report({'WARNING'}, f"Skapade fönsterkomponent: {name} (markera en vägg för att placera den)")
